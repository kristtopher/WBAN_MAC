        ...
        if (crc) {
            ...
            if (isAckPack(recPackage)) {
                ...
                // RTS Received
                    ...
                    ((!conection_state) || addr0[0] == recPackage[7]) &&
                ...
                // send CTS
                generate_and_send_cmd(request, 10, packRequest , addr0, cmd_cts);
                dout<<"Sending CTS "<< cmd_cts<<std::endl;
                printf("%d",cmd_cts);
                dout<<"troca de canal aguradando synk\n"<< std::endl;
